	...
	var postData = localStorage.getItem('tempPostData');
			...
			$.each(jsonObj.posts, function(i, field){
				...
				$("#output").append('<div class="item col-50">'+
										'<a class="blog-link" id="'+frompost_id+'" href='+base_wp_url+'/?json=get_post&post_id='+frompost_id+'">'+
		...
		/*function isUpdated(jsonTimestamp) {
		  var storedTimestamp = localStorage.getItem("modified");
		  if (storedTimestamp === null) {
			localStorage.setItem("modified", jsonTimestamp);
			return false;
		  }
		...
		//	var postData = localStorage.getItem('tempPostData');
			...
			$.each(jsonObj.posts, function(i, field){
				...
				var thumbnail=field.thumbnail;
					...
					/*$(".view").append('<div data-page="single'+id+'" class="page cached">'+
				...
				/* function displayPost() {
						...
						$$("#"+id+".blogposts").show();
						$$('.left a, .left .back').show();
						});
					
					}*/
  	            //$("#output").append("<li><h4>"+title+"<h4><p>"+date+"</p></li>");
				...
				/*$(".view").on('click', function(){
					localStorage.setItem('post_id', frompost_id);
				});
				*/
...
function getPostContent() {
  	...
  	myApp.showIndicator();
		var post_id = localStorage.getItem('temppost_id');
  		var url= base_wp_url+"/?json=get_post&post_id="+post_id;
  		$.getJSON(url,function(result){
			console.log(result);
 ...
 /* 
  function getPosts1() {
    (function() {
      var postsAPI = base_wp_url+"/wp-json/wp/v2/posts";
      $.getJSON( postsAPI, {
        type: "post",
      //  tagmode: "any",
        format: "jsonp"
        ...
        //  $("#output").append(data);
          $.each( data.items, function( i, item ) {
            ...
            $( "h2" ).html( item.media.m ).appendTo( "#output" );
            $("#output").append(data);
            if ( i === 3 ) {
